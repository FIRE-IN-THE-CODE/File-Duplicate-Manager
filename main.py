...
def usage():
    ...
    print ('\n')
    print ('Description: ')
    print ('	Helps to clear storage space by erasing duplicate files in a directory.')
...
def list_generator(new_list):
    print (new_list)			#For testing only
    new_list = []
    for x in new_list:
        dir_detect(x)
    return new_list
...
def dir_detect(sus_variable):
    print (sus_variable)			#For testing only
...
def main():
    ...
    if len(excluded_folder) > 0:		#Debugging - keep an eye on this condition because the delete function wasn't made yet
    ...
    print ('Options: 1 - delete the file in the first directory, 2 - delete the file in the second directory, 3 - do nothing')
